...
def _auth() -> httpx.BasicAuth | None:
    password = (settings.opencode_server_password or "").strip()
    if not password:
        return None
    username = (settings.opencode_server_username or "opencode").strip() or "opencode"
...
def _message_body(
    ...
    *,
    ...
    return {
        "model": {"providerID": settings.opencode_provider_id, "modelID": model_id},
        "agent": agent,
