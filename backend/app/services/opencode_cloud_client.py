...
def _map_openai_error(exc: OpenAIError) -> OpenCodeError:
    ...
    if isinstance(exc, APITimeoutError):
        return OpenCodeError(
            f"הבקשה ל-AI ארכה יותר מ-{int(settings.opencode_timeout_seconds)} שניות.",
            retryable=False,
        )
...
def _client(timeout: float) -> AsyncOpenAI:
    return AsyncOpenAI(
        api_key=settings.opencode_api_key.strip(),
        base_url=settings.opencode_api_base_url.rstrip("/"),
        timeout=timeout,
    )
